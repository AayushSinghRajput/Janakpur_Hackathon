...
const NGODashboard = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const action_taken = reportList.filter(r => 
          r.status === "action_taken" || r.status === "resolved"
        ).length;
        const resolved = reportList.filter(r => r.status === "resolved").length;
        const emergency = reportList.filter(r => r.urgencyLevel === "Emergency").length;
        const normal = reportList.filter(r => r.urgencyLevel === "Normal").length;

        setStats({ total, pending, under_review, action_taken, resolved, emergency, normal });
  ...
  useEffect(() => {
    ...
    if (urgencyFilter !== "all") {
      filtered = filtered.filter(report => report.urgencyLevel === urgencyFilter);
    }
  ...
  const handleMarkAsResolved = async () => {
    ...
    try {
      ...
      const pending = updatedReportsList.filter(r => r.status === "pending").length;
      const under_review = updatedReportsList.filter(r => r.status === "under_review").length;
      const action_taken = updatedReportsList.filter(r => 
        r.status === "action_taken" || r.status === "resolved"
      ).length;
      const resolved = updatedReportsList.filter(r => r.status === "resolved").length;
      const emergency = updatedReportsList.filter(r => r.urgencyLevel === "Emergency").length;
      const normal = updatedReportsList.filter(r => r.urgencyLevel === "Normal").length;
      ...
      alert(error.message || 'Failed to update report status. Please try again.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900">
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-red-400">{stats.emergency}</p>
                </div>
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-purple-400">{stats.action_taken}</p>
                </div>
              ...
              <div className="mt-4">
                <div className="h-1 bg-gray-700 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${stats.total ? (stats.action_taken / stats.total) * 100 : 0}%` }}
                  ></div>
                </div>
              </div>
            </div>
            ...
            {(searchTerm || statusFilter !== 'all' || urgencyFilter !== 'all') && (
              <div className="mt-4 pt-4 border-t border-gray-700">
                <div className="flex items-center flex-wrap gap-2">
                  ...
                  {urgencyFilter !== 'all' && (
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${urgencyFilter === 'Emergency' ? 'bg-red-900/30 text-red-300 border-red-700/30' : 'bg-blue-900/30 text-blue-300 border-blue-700/30'}`}>
                      Urgency: {urgencyFilter}
                      <button 
                        onClick={() => setUrgencyFilter('all')}
                        className={`ml-2 ${urgencyFilter === 'Emergency' ? 'text-red-200 hover:text-white' : 'text-blue-200 hover:text-white'}`}
                      >
                        ×
                      </button>
                    </span>
          ...
          <div className="bg-gray-800/90 backdrop-blur-sm rounded-2xl border border-gray-700 max-w-6xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-8">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center space-x-3">
                  <div className={`w-3 h-3 rounded-full ${selectedReport.urgencyLevel === 'Emergency' ? 'bg-red-500 animate-pulse' : 'bg-blue-500'}`}></div>
                                ...
                                <div className="relative w-full h-96">
                                  <img 
                                    ...
                                    onError={(e) => {
                                      ...
                                      e.target.src = `https://via.placeholder.com/1200x400/1f2937/9ca3af?text=${encodeURIComponent(fileName)}`;
                                    }}
                                  />
                                  {/* Overlay */}
                                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
                                    <div className="flex items-center space-x-2 text-white">
                                      ...
                                      <span className="text-lg font-medium">Click to view fullscreen</span>
                                    </div>
                                  </div>
                                </div>
                              </>
                              ...
                              <>
                                ...
                                <div className="relative w-full h-64 flex flex-col items-center justify-center p-8">
                                  ...
                                  <div className="mt-6 text-center px-4">
                                    <p className="text-xl text-white font-medium truncate w-full">{fileName}</p>
                                    <p className="text-gray-300 mt-2">Click to play video in fullscreen</p>
                                  </div>
                                </div>
                              </>
                            ) : (
                              <>
                                ...
                                <div className="relative w-full h-48 flex flex-col items-center justify-center p-8">
                                  ...
                                  <div className="mt-6 text-center">
                                    <p className="text-xl text-white font-medium truncate w-full px-8">{fileName}</p>
                                    <p className="text-gray-300 mt-2">Click to download file</p>
                                  </div>
                                </div>
                              </>
                            )}
                              ...
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm text-gray-400 mt-1">
                                    {fileType.charAt(0).toUpperCase() + fileType.slice(1)} file • Click to preview
                                  </p>
                                </div>
                              </div>
                              <button 
                      ...
                      <div className="p-2">
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-400 mt-1">Standard report format</div>
                          </div>
                        </button>
                        <button
                          onClick={() => {
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-400 mt-1">View & print directly</div>
                          </div>
                        </button>
                        <button
                          onClick={() => {
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-xs text-gray-400 mt-1">Plain text format</div>
                          </div>
                        </button>
                      </div>
              ...
              {selectedReport.status === 'resolved' && (
                <div className="mt-6 p-4 bg-green-900/20 border border-green-700/30 rounded-xl">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-green-300 font-medium">This report has been marked as resolved</p>
      ...
      {evidenceModalOpen && selectedReport && selectedReport.evidenceUrls && selectedEvidenceIndex !== null && (
          ...
          <div className="relative w-full h-full flex flex-col">
            ...
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-10">
              <div className="px-4 py-2 bg-black/70 rounded-full text-white text-sm">
                {selectedEvidenceIndex + 1} / {selectedReport.evidenceUrls.length}
              </div>
            </div>
            ...
            <div className="flex-1 flex items-center justify-center p-4">
              <div className="max-w-6xl max-h-full w-full">
                {getFileType(selectedReport.evidenceUrls[selectedEvidenceIndex]) === 'image' ? (
                ...
                ) : getFileType(selectedReport.evidenceUrls[selectedEvidenceIndex]) === 'video' ? (
                    ...
                    <div className="relative pt-[56.25%]"> {/* 16:9 Aspect Ratio */}
                      <video
                        ...
                        onError={(e) => {
                          ...
                          errorDiv.className = 'absolute inset-0 flex flex-col items-center justify-center bg-gray-900 rounded-lg';
                          errorDiv.innerHTML = `
                  ...
                  <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl p-12 border border-gray-700 max-w-2xl mx-auto">
                    <div className="text-center">
                      ...
                      <h4 className="text-2xl font-bold text-white mb-4">
                        {selectedReport.evidenceUrls[selectedEvidenceIndex].split('/').pop()}
                      </h4>
                      <p className="text-gray-400 text-lg mb-8">
                        This file type cannot be previewed. Please download it to view.
                      </p>
                      <button
                        onClick={() => window.open(selectedReport.evidenceUrls[selectedEvidenceIndex], '_blank')}
            ...
            <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10">
              <div className="px-6 py-3 bg-black/70 backdrop-blur-sm rounded-xl text-white text-center">
                ...
                <p className="text-gray-300 text-sm mt-1">
                  {getFileType(selectedReport.evidenceUrls[selectedEvidenceIndex]).toUpperCase()} • 
